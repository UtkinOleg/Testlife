...
if (!empty($action)) 
{
  ...
  $gst3 = mysqli_query($mysqli,"SELECT name, ownerid, checkdate1 FROM projectarray WHERE id='".$paid."'");
...
if ((defined("IN_SUPERVISOR") and $projectarray['ownerid'] == USER_ID) or defined("IN_ADMIN")) 
{
  ...
  if (!empty($email))
  {
   ...
   $tot2 = mysqli_query($mysqli,"/*" . MYSQLND_QC_ENABLE_SWITCH . "*/" . "SELECT count(*) FROM prousers WHERE proarrid='".$paid."' AND email='".$email."'");
   $tot2cnt = mysqli_fetch_array($tot2);
   ...
   {  
    ...
    $query = "INSERT INTO prousers VALUES (0,
                                        $userid,
                                        $paid,
                                        '$email');";
    if(mysqli_query($mysqli,$query))
    {
       ...
       $title = "Приглашение от expert03.ru";
       $body = msghead($fio, $site);
       $body .='<p>Вам отправлено приглашение от супервизора <strong>'.USER_FIO.'</strong> для создания проекта <strong>'.$name.'</strong> в системе <a href="'.$site.'" title="Экспертная система оценки проектов" target="_blank">'.$site.'</a>.</p>';
       if ($userid!=0)
        $body.='<p>Вам необходимо в срок до '.data_convert ($projectarray['checkdate1'], 1, 0, 0).' в личном кабинете на сайте создать проект <strong>'.$name.'</strong>. В зависимости от условий проекта, Вам может быть предложено также пройти онлайн тестирование.</p>';
       else
        $body.='<p>Вам необходимо в срок до '.data_convert ($projectarray['checkdate1'], 1, 0, 0).' <a href="'.$site.'" title="Экспертная система оценки проектов" target="_blank">зарегистрироваться</a> на данном сайте (кнопка "Вход - Регистрация" или автоматическая регистрация через любую социальную сеть) и создать проект <strong>'.$name.'</strong> в личном кабинете. В зависимости от условий проекта, Вам может быть предложено также пройти онлайн тестирование.</p>';
       $body .= msgtail($site);
  ...
  echo 'parent.closeFancyboxAndRedirectToUrl("'.$site.'/prousers&paid='.$paid.'");';
  echo '</script>';
  exit();
 ...
 jQuery(document).ready(function() {
    $('#adds').submit(function()
    {
     ...
     } else 
     {
      ...
      $("#info1").addClass("ui-state-error");
      $("#info2").empty();
      $("#info2").append('Укажите адрес участника и нажмите кнопку <button id="helpb"><i class="fa fa-plus fa-lg"></i> Добавить участника</button>');
      $("#helpb").button();
...
<table border=0 cellpadding=0 cellspacing=0 width='100%' height='100%' bgcolor='#ffffff'><tr><td align='center'>
    <div class="ui-widget">            	   
      <div id='info1' class="ui-state-highlight ui-corner-all" style="padding: 0 .7em; font: 14px / 1.4 'Helvetica', 'Arial', sans-serif;">                    
        <p>      
          <div id="info2">Укажите адрес электронной почты нового участника.</div>    
        </p>            	   
      </div>
    </div>
    <p></p>  
...
<table class="bodytable" border="0" width='100%' height='100%' cellpadding=3 cellspacing=3 bordercolorlight=gray bordercolordark=white>
    ...
    <tr>    
     <td>
        <form id="adds" action="addprouser" method="post">
...
<div id="buttonset">
            <p></p>
            <button id="add" onclick="addemail();"><i class="fa fa-plus fa-lg"></i> Добавить участника</button>  

<?php
  ...
  if ($member['status']!='finalized' or $member['status']!='published') 
  {
   ...
   if ($projectarray['addcomment']==1) {
    echo "<p><table width='800' class=bodytable border='0' cellpadding=3 cellspacing=3 bordercolorlight=gray bordercolordark=white>";
    echo "<tr class='tableheader'><td witdh='300'><p class=help>Новый комментарий</p></td></tr>";   
    ?>
    <form action="addcomment" method="post" enctype="multipart/form-data">
    <input type=hidden name=action value=post>
    <input type=hidden name=back value=<? echo $back; ?>>
    <input type=hidden name=id value='<? echo $member['id']; ?>'>
    <?
     echo"<tr><td><textarea name='content' style='width:100%'></textarea></td></tr>
     <tr align='center'><td>
     <input type='submit' value='Добавить комментарий (замечание)'>
...
//  echo "<tr><td width='100' align='center'><b><a href='index.php?op=".$back."'>Назад</a></b></td></tr>"; 
//  echo "</table></p><br>";
//  <p align="center"><input type="button" name="close" value="Назад" onclick="document.location='<? echo $site; /index.php?op=<? echo $back; '"></p>
